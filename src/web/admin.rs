...
use crate::web::{
    AppState, AuthUser, GlossaryTermRow, JournalReferenceRow, JournalTopicRow,
    JournalTopicScoreRow, auth, escape_html, fetch_glossary_terms, fetch_journal_references,
    fetch_journal_topic_scores, fetch_journal_topics, render_footer,
};
use crate::{
    config::{
        DocxTranslatorModels, DocxTranslatorPrompts, GraderModels, GraderPrompts, SummarizerModels,
        SummarizerPrompts, update_docx_models, update_docx_prompts, update_grader_models,
...
#[derive(Deserialize)]
pub(crate) struct SummarizerModelForm {
...
#[derive(Deserialize)]
pub(crate) struct SummarizerPromptForm {
...
#[derive(Deserialize)]
pub(crate) struct DocxModelForm {
    translation_model: String,
    #[serde(default)]
    redirect: Option<String>,
}
...
#[derive(Deserialize)]
pub(crate) struct DocxPromptForm {
    en_to_cn: String,
    cn_to_en: String,
    #[serde(default)]
    redirect: Option<String>,
...
#[derive(Deserialize)]
pub(crate) struct GraderModelForm {
...
#[derive(Deserialize)]
pub(crate) struct GraderPromptForm {
...
pub async fn dashboard(
    ...
    let Some(token_cookie) = jar.get(auth::SESSION_COOKIE) else {
...
pub async fn save_summarizer_models(
    State(state): State<AppState>,
    jar: CookieJar,
...
pub async fn save_summarizer_prompts(
    State(state): State<AppState>,
    jar: CookieJar,
...
pub async fn save_docx_prompts(
    State(state): State<AppState>,
    jar: CookieJar,
...
pub async fn save_grader_models(
    State(state): State<AppState>,
    jar: CookieJar,
...
pub async fn save_grader_prompts(
    State(state): State<AppState>,
    jar: CookieJar,
